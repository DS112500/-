...
#define      Beacon1    GPIO_PIN_55
#define      Beacon2    GPIO_PIN_56
#define      Beacon3    GPIO_PIN_45
...
void TIMER_8M_INT(void)
{
    ...
    Set_Timer_clear();
    Set_Timer_Init(8 * 10000*10000); //  1.25ms
...
char  UART_RX_BUF[USART_REC_LEN]; //接收缓冲,最大USART_REC_LEN个字节.末字节为换行符 
uint8_t sec=0;
...
void intc_handler(void)
{
    ...
    if (IntReg & UART0_INT_OUT) // Uart0
    {
        receive_Data = UART_ReceiveData(UART0);                    // 接收数据
          if (receive_Data == '1')
          {
               UART_SendString(UART1,"3");
               delay_s(1);
          }
          if (receive_Data == '2')
          {
              UART_SendString(UART1, "4");
              delay_s(1);
          }
          if (receive_Data == '3')
          {
              UART_SendString(UART1, "5");
              delay_s(1);
          }
