...
void TIM_StructClose(TIM_InitTypeDef* TIM_InitStruct)
{
    /* Set the default configuration */
    TIM_InitStruct->Clock_Freq = 8000000;						//系统时钟
    TIM_InitStruct->TIME_START = 0x00;							//启动定时器
    TIM_InitStruct->TIME_INTEN = 0x00;						//中断使能
}

void timer_close()
{
    TIM_InitTypeDef TIM_InitStruct1;
    TIM_StructClose(&TIM_InitStruct1);
}
